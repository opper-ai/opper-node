 ...
 */
export function spansSubmitSpanFeedbackSpansSpanIdFeedbackPost(
  client: OpperCore,
  spanId: string,
...
> {
  const input: operations.SubmitSpanFeedbackSpansSpanIdFeedbackPostRequest = {
    spanId: spanId,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.SubmitSpanFeedbackSpansSpanIdFeedbackPostRequest$outboundSchema
        .parse(value),
  ...
  const body = encodeJSON("body", payload.SubmitFeedbackRequest, {
    explode: true,
  });

  const pathParams = {
    span_id: encodeSimple("span_id", payload.span_id, {
      explode: false,
  ...
  const context = {
    ...
    operationID: "submit_span_feedback_spans__span_id__feedback_post",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.SubmitFeedbackResponse$inboundSchema),
