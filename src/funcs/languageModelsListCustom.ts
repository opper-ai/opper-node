 ...
 */
export function languageModelsListCustom(
  client: OpperCore,
...
> {
  const input: operations.ListCustomModelsModelsCustomGetRequest = {
    offset: offset,
    limit: limit,
  };
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.ListCustomModelsModelsCustomGetRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/models/custom")();
  ...
  >(
    M.json(
      200,
      models.PaginatedResponseListCustomModelsResponseItem$inboundSchema,
    ),
