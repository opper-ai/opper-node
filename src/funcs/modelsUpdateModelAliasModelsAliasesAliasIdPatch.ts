 ...
 */
export function modelsUpdateModelAliasModelsAliasesAliasIdPatch(
  client: OpperCore,
  aliasId: string,
...
> {
  const input: operations.UpdateModelAliasModelsAliasesAliasIdPatchRequest = {
    aliasId: aliasId,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.UpdateModelAliasModelsAliasesAliasIdPatchRequest$outboundSchema
        .parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateModelAliasRequest, {
    explode: true,
  });

  const pathParams = {
    alias_id: encodeSimple("alias_id", payload.alias_id, {
      explode: false,
  ...
  const context = {
    ...
    operationID: "update_model_alias_models_aliases__alias_id__patch",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdateModelAliasResponse$inboundSchema),
