 ...
 */
export function datasetsEntriesUpdate(
  client: OpperCore,
  datasetId: string,
  entryId: string,
...
> {
  const input:
    operations.UpdateDatasetEntryDatasetsDatasetIdEntriesEntryIdPatchRequest = {
      datasetId: datasetId,
      entryId: entryId,
  ...
  const body = encodeJSON("body", payload.UpdateDatasetEntryRequest, {
    explode: true,
  });
  ...
  const path = pathToFunc("/datasets/{dataset_id}/entries/{entry_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdateDatasetEntryResponse$inboundSchema),
