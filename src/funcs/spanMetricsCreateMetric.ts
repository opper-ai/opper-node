 ...
 */
export function spanMetricsCreateMetric(
  client: OpperCore,
  spanId: string,
...
> {
  const input: operations.CreateMetricSpansSpanIdMetricsPostRequest = {
    spanId: spanId,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.CreateMetricSpansSpanIdMetricsPostRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.CreateSpanMetricRequest, {
    explode: true,
  });
  ...
  const context = {
    ...
    operationID: "create_metric_spans__span_id__metrics_post",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.CreateSpanMetricResponse$inboundSchema),
