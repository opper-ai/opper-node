...
> {
  const input: operations.GetTraceTracesTraceIdGetRequest = {
    traceId: traceId,
  };
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.GetTraceTracesTraceIdGetRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/traces/{trace_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.GetTraceResponse$inboundSchema),
