 ...
 */
export function spanMetricsUpdateMetric(
  client: OpperCore,
  spanId: string,
  metricId: string,
...
> {
  const input: operations.UpdateMetricSpansSpanIdMetricsMetricIdPatchRequest = {
    spanId: spanId,
    metricId: metricId,
  ...
  const body = encodeJSON("body", payload.UpdateSpanMetricRequest, {
    explode: true,
  });

  const pathParams = {
    metric_id: encodeSimple("metric_id", payload.metric_id, {
      explode: false,
  ...
  const path = pathToFunc("/spans/{span_id}/metrics/{metric_id}")(pathParams);
  ...
  const context = {
    ...
    operationID: "update_metric_spans__span_id__metrics__metric_id__patch",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdateSpanMetricResponse$inboundSchema),
