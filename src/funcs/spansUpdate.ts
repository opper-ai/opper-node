...
> {
  const input: operations.UpdateSpanSpansSpanIdPatchRequest = {
    spanId: spanId,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.UpdateSpanSpansSpanIdPatchRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateSpanRequest, { explode: true });
  ...
  const path = pathToFunc("/spans/{span_id}")(pathParams);
  ...
  const context = {
    ...
    operationID: "update_span_spans__span_id__patch",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdateSpanResponse$inboundSchema),
