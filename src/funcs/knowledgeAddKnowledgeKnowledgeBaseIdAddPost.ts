 ...
 */
export function knowledgeAddKnowledgeKnowledgeBaseIdAddPost(
  client: OpperCore,
...
> {
  const input: operations.AddKnowledgeKnowledgeBaseIdAddPostRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.AddKnowledgeKnowledgeBaseIdAddPostRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.AddRequest, { explode: true });
  ...
  const path = pathToFunc("/knowledge/{knowledge_base_id}/add")(pathParams);
  ...
  const context = {
    ...
    operationID: "add_knowledge__knowledge_base_id__add_post",
    oAuth2Scopes: [],
